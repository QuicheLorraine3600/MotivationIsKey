...
export default class FakeMessage extends Command {
	...
	override async execute(bot: Bot, interaction: Interaction) {
		...
		const targeteUser = interaction.options.getUser("target", true)
		const message = interaction.options.getString("message", true)

		const channel = interaction.channel
		if (channel instanceof TextChannel) {
			sendWebhookMessageToChannel(channel, targeteUser.displayName ?? "", targeteUser.avatarURL() ?? "", message).then(() => {
